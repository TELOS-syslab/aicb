...
# 定义一个工作项类； fth： 推理不需要各种backwards； dp的概念也有所不同； 参考vidur
class Work_Item:
...
class Vidur_SIMAI_workload:
    ...
    def get_model_details(self):  # 获取模型的详细信息
        ...
        def traverse_model(model):  # 定义递归遍历模型的方法
            ...
            if self.args.enable_sequence_parallel:  # 如果启用了序列并行
                ...
                # ):
                if (
                    isinstance(model, SarathiColumnLinear)  # 如果是列线性层
                    or isinstance(model, SarathiRowLinear)  # 或行线性层
                    or isinstance(model, SarathiEmbedding)  # 或嵌入层
                    or isinstance(model, FusedLayernorm)  # 或归一化层
                ):
    ...
    def workload_generate_aiob(self):
        ...
        # 初始化计算时间为0
        compute_time = 0
        # 计算张量并行通信大小 = 2 * 微批次数 * 序列长度 * 隐藏层大小
        # fth: 需要换成sarathi 推理的tp_comm_size 公式； 得去sarathi那边找
        tp_comm_size = (
...
if __name__ == "__main__":  # 如果脚本作为主程序运行
    ...
    if args.aiob_enable:  # 如果启用了 AI-O部门
        ...
        work = Vidur_SIMAI_workload(  # 初始化 SIMAI_workload 实例
    ...
    # print(args)  # 注释掉的打印参数
        ...
        work = Vidur_SIMAI_workload(model, args, None)  # 初始化 SIMAI_workload 实例，传入 None 作为计算缓存
        name_layers = work.workload_generate()  # 生成工作负载
        work.dump_file(filepath)  # 将工作负载保存到文件
