...
class SarathiRowLinear(MockedModel):
        ...
        if self.computation_enable:  # 如果计算启用
            workloads.append(  # 将计算工作负载记录到workloads中
                LogItem(  # 创建日志条目
                    ...
                    stage="forward.SarathiRowLinear." + self.name,  # 阶段信息
                )
            )
...
class SarathiColumnLinear(MockedModel):
            ...
            if self.sequence_parallel_enabled:  # 如果启用了序列并行
                workloads.append(  # 向工作负载中添加日志条目
                    LogItem(  # 创建日志条目，表示通信操作
                        ...
                        stage="forward.SarathiColumnLinear",  # 阶段信息
                    )
                )
    ...
    # 推理没有backward； 可以不用考虑，不要进行backward
            ...
            if self.sequence_parallel_enabled:  # 如果启用了序列并行
                workloads.append(  # 向工作负载中添加日志条目
                    LogItem(  # 创建日志条目，表示all_gather操作
                        ...
                        stage="backward.SarathiColumnLinear",  # 阶段信息
                    )
                )
        ...
        if self.computation_enable:  # 如果启用了计算
            workloads.append(  # 向工作负载中添加计算操作的日志条目
                LogItem(  # 创建日志条目，表示计算操作
                    ...
                    # stage="backward.MegatronColumnLinear." + self.name,  # 阶段信息
                    stage="backward.SarathiColumnLinear." + self.name,  # 阶段信息
                )
            )
        if self.tensor_model_parallel_size > 1:  # 如果tensor模型并行大小大于1
...
class SarathiAttention(MockedModel):
    def __init__(  # 初始化方法，接收多个参数来配置注意力层
        ...
        self.attention_dense = SarathiRowLinear(
...
class SarathiMlp(MockedModel):
    def __init__(  # 初始化方法，接收多个参数来配置MLP层
        ...
        self.dense_h_to_4h = SarathiColumnLinear(
            hidden_size,  # 输入大小为隐藏层大小
        ...
        self.dense_4h_to_h = SarathiRowLinear(
...
class SarathiTransformorLayer(MockedModel):
    def __init__(  # 初始化方法，包含多个输入参数
        ...
        self.attention = SarathiAttention(
            num_attention_heads,
            hidden_size,
            tp,
            seq_len,
...
class SarathiEmbedding(MockedModel):  # 定义一个 MegatronEmbedding 类，继承自 MockedModel
        ...
        if self.tensor_model_parallel_size > 1:  # 如果张量并行度大于1
            workloads.append(  # 向工作负载列表中添加一个通信项
                LogItem(
                    ...
                    stage="forward.SarathiEmbedding",  # 设置当前阶段为前向传播
                )
            )
        return workloads  # 返回工作负载
...
class SarathiModel(MockedModel):
    ...
    def __init__(
        ...
        self.embedding = SarathiEmbedding(
        ...
        self.final_norm = SarathiColumnLinear(
            config.hidden_size,  # 隐藏层大小
